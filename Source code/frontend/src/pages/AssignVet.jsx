...
const STEPS = {
  VIEW_DOCTOR_SCHEDULE: 'viewDoctorSchedule',
...
const AssignVet = () => {
    ...
    const [currentStep, SetCurrentStep] = useState(STEPS.VIEW_SCHEDULE);
  ...
  const handleStepChange = (step) => {
    if (step === STEPS.ASSIGN_DOCTOR) {
      SetSelectedDoctor(null);
      SetIsFull(true);
    } else if (step === STEPS.VIEW_DOCTOR_SCHEDULE) {
      SetIsFull(false);
    }
    SetCurrentStep(step);
  };
...
//    { name: "Nguyễn Văn I", degree: "Thạc sĩ", schedule: "Thứ 2, 5", EmployeeID: "E9" }
  ...
  const StepButton = ({ step, label }) => {
    const isActive = currentStep === step || 
                    (step === STEPS.ASSIGN_DOCTOR && currentStep === VIEW_DOCTOR_SCHEDULE);
    ...
    <div className="assignvet-page">
      ...
      <div className="main-content">
            <div className="step-buttons">
                <StepButton step={STEPS.VIEW_SCHEDULE} label="Xem lịch" />
                <StepButton step={STEPS.ASSIGN_DOCTOR} label="Đặt lịch" />
                <StepButton step={STEPS.VIEW_DOCTOR_SCHEDULE} label="Chọn bác sĩ" />
            </div>
